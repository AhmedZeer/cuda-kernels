...
// 32-thread warp access to aligned memory blocks (e.g., 32 * 16 bytes = 512 bytes).
// Single Instruction Multiple (SIMD).
...
__global__ void element_wise_add_f32(float *a, float *b, float *c, uint N){
...
// gridDim(N/256), blockDim(256)
// a: Nx1, b: Nx1, c: Nx1, c = add(a,b)
__global__ void element_wise_add_f16(float *a, float *b, float *c, uint N){
  uint idx = blockIdx.x  * blockDim.x + threadIdx.x;
...
// a: Nx1, b: Nx1, c: Nx1, c = add(a,b)
__global__ void element_wise_add_f16x2(float *a, float *b, float *c, uint N){
...
__global__ void element_wise_add_f16x8(float *a, float *b, float *c, uint N){
...
// instruction.
__global__ void element_wise_add_fp16_packed(float *a, float *b, float *c, int N){
  ...
  void add_#packed_type(torch::Tensor a, torch::Tensor b, torch::Tensor c){ \
    ...
    if(ndim != 2){ \
      ...
      element_wise_add_##packed_type<<<gridDim, blockDim>>>( \
          ...
          reinterpert_cast<element_type*>(c.data_ptr()), \
          N \
      ) \
    } else { \
...
BIND_ELM_ADD(f16, torch::Half, half, 1);
